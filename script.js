document.addEventListener('DOMContentLoaded', function() {
    ...
    function getQuizData() {
      ...
      for (let i = 0; i < questionInputs.length; i++) {
        ...
        const question = questionInput.querySelector(`#question${i + 1}`).value;
        const options = [];
        for (let j = 1; j <= 4; j++) {
          options.push(questionInput.querySelector(`#option${i + 1}-${j}`).value);
        }
        const correctAnswer = questionInput.querySelector(`#correctAnswer${i + 1}`).value;
    ...
    function generateQuizLink(quizData) {
      ...
      const quizLink = `${window.location.href.split('?')[0]}?quizid=${quizID}&data=${encodedQuizData}`;
      return quizLink;
    }
    ...
    function submitQuizUser2() {
        const selectedOptionsUser2 = document.querySelectorAll('input[type="radio"]:checked');
        let scoreUser2 = 0;
        ...
        selectedOptionsUser2.forEach(function(selectedOption) {
            const questionIndex = parseInt(selectedOption.name.replace('question', ''));
            const selectedValue = selectedOption.value;
            const correctAnswer = window.quizData[questionIndex].answer;
            if (selectedValue === correctAnswer) {
                scoreUser2++;
            }
        });

        resultContainerUser2.textContent = `You scored ${scoreUser2} out of ${window.quizData.length}!`;
